...
def update_workflow_from_makefile(c, workflow_name):
    ...
    update_workflow_xml(c, params["WORKFLOW_NAME"], params["TOOL_FOLDER_NAME"], params["WORKFLOW_VERSION"], workflow_name, "production")
    update_tools(c, params["TOOL_FOLDER_NAME"], params["WORKFLOW_VERSION"], workflow_name, "production")
...
def update_workflow_xml(c, workflow_name, tool_name, workflow_version, base_dir=".", production_str=""):
    ...
    if production:
        ...
        c.sudo("mkdir -p /ccms/workflows/{}/versions/{}".format(workflow_name, workflow_version), user=c["env"]["production_user"]["workflow"], pty=True)
    else:
...
def update_tools(c, workflow_name, workflow_version, base_dir=".", production_str=""):
    ...
    if production:
        c.sudo("mkdir -p /data/cluster/tools/{}/{}".format(workflow_name, workflow_version), user=c["env"]["production_user"]["tool"], pty=True)
    else:
    ...
    update_folder(c, local_path, final_path, production=production, production_user=c["env"]["production_user"]["tool"])
...
#Update File
def update_file(c, local_path, final_path, production=False, production_user=None):
    if production:
...
def update_folder(c, local_path, final_path, production=False, production_user=None):
